...
**GATE 20 (Loops 96-100) — RESOLVED.** The Boss replied **"Continue loop
101 to 105"** and, in the same message, supplied both design decisions
this gate report (and 8 before it, since Gate 12) had been asking for:
RISK-32 (reopen authority) is MAINTENANCE_MANAGER only; RISK-33
(complainant disagreement) is a complainant-raises/Executive-acknowledges
interaction. Full detail in `APPROVAL_REPORT_LOOP_96_100.md`.

**GATE 21 (Loops 101-105) — AWAITING BOSS.** Both decisions implemented
server-side, migration-first, exactly as given, with the actual "no
unilateral closure" enforcement living inside `transition_case` itself,
not just the UI. **RISK-32 and RISK-33 are both now RESOLVED** in
`RISK_REGISTER.md` — for the first time since Gate 12, this report has no
outstanding business-rule design question to re-raise. One real gap is
explicitly acknowledged rather than silently skipped: browser-level e2e
coverage for the Reopen button and the two new dispute forms does not
exist yet (the RPC-level authority is fully tested; the UI-wiring layer
is only manually verified via `tsc`/`eslint`/`next build`, not an
automated browser test) — writing it safely needs a multi-step UI walk
this sandbox cannot verify locally, so it was deliberately deferred
